...
const Projects = () => {
  ...
  const handleClickArrows = (number: number) => {
    setProjectIndex((prev) => prev + number)
    return projectIndex
  }
  const handleClickIndex = (number:number) => {
  ...
  return (
    <section id="projects">
      ...
      <ul id="projectsIndex-ul">
        {projectsData.map((project, index) => <li value={index} className={(index === projectIndex) ? 'projectIndex-ul__li focused' : 'projectIndex-ul__li'} onClick={() => handleClickIndex(index)}> 
        </li>)}
      </ul>
      <ul id="projects-container">
        <button type="button" disabled={projectIndex === 0} id="prev-button" onClick={() => {
          handleClickArrows(-1) 
        }}>
        ...
        <button type="button" disabled={projectIndex === projectsData.length - 1} id="next-button" onClick={() => {
          handleClickArrows(1) 
        }}>
